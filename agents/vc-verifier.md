...
Read `${CLAUDE_PLUGIN_ROOT}/reference/checklist.md` (if that variable is not expanded, use `~/.claude/skills/vibecheck/reference/checklist.md`) in full before starting. Your spawn prompt
...
**4. Check for omission.** Did any auditor skip a check ID, silently narrow its scope, or mark
...
**5. Sanity-check the N/As.** An N/A is correct when the capability genuinely doesn't exist in
